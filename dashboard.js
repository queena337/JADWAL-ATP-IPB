...
function showAppAlert(message) {
  ...
  const isError = /error|gagal|tidak|wajib|belum|❌|⚠️/i.test(text);
  const icon = document.getElementById("appDialogIcon");
  document.getElementById("appDialogTitle").textContent = isError
    ? "Perlu diperiksa"
    : "Berhasil diproses";
  document.getElementById("appDialogMessage").textContent = text.replace(
    /^[✅⚠️❌🔄📄📊🖨️📝]\s*/u,
    "",
  );
...
function showAppConfirm(message, onConfirm, actionLabel = "Hapus") {
  const dialog = document.getElementById("appConfirmDialog");
  if (!dialog) return;
  document.getElementById("appConfirmMessage").textContent = message;
  const actionButton = document.getElementById("appConfirmAction");
...
function sinkronkanKunjunganKeCapaian() {
  capaianMingguanData = [];
  const dataKunjungan = kunjunganData.filter(
    (item) => item.jumlahPengunjung && item.jumlahPengunjung > 0,
  );
  dataKunjungan.forEach((item) => {
    const existing = capaianMingguanData.find(
      (d) => d.tanggal === item.tanggal && d.instansi === item.instansi,
    );
    if (existing) {
...
function simpanSemuaData() {
  try {
    ...
    localStorage.setItem(
      "capaianMingguanData",
      JSON.stringify(capaianMingguanData),
    );
    ...
    if (window.simpanDataRealtime) {
      window.simpanDataRealtime({
        masterRuangan,
        masterTempat,
        masterPic,
        masterInstansi,
...
window.terapkanDataRealtimeAdmin = function (remoteData) {
  ...
  programData = remoteData.programData || [];
  eventData = remoteData.eventData || [];
  capaianData = remoteData.capaianData || [];
  capaianMingguanData = remoteData.capaianMingguanData || [];
  nextKunjunganId = remoteData.nextKunjunganId || 1;
  nextRuangId = remoteData.nextRuangId || 1;
  nextBalaiId = remoteData.nextBalaiId || 1;
  nextProgramId = remoteData.nextProgramId || 1;
  nextEventId = remoteData.nextEventId || 1;
...
window.getDataRealtimeAdmin = function () {
  return {
    masterRuangan,
    masterTempat,
    masterPic,
...
function muatSemuaData() {
  try {
    ...
    if (savedCapaianMingguan)
      capaianMingguanData = JSON.parse(savedCapaianMingguan);
    if (savedNextKunjunganId)
      nextKunjunganId = parseInt(savedNextKunjunganId) || 3;
...
const pageTitles = {
  ...
  kalender: {
    ...
    subtitle: "Kelola event per tanggal",
  },
  capaian: {
    title: "Capaian Pengunjung",
    subtitle: "Kelola data capaian pengunjung",
  },
...
function renderRingkasanJadwal() {
  ...
  const kunjunganHariIni = kunjunganData.filter(
  ...
  if (tbodyKunjungan) {
    if (kunjunganHariIni.length === 0) {
      ...
      tbodyKunjungan.innerHTML = kunjunganHariIni
        .map(
          (item, index) => `
                <tr>
                    <td>${index + 1}</td>
  ...
  if (tbodyRuang) {
    if (ruangHariIni.length === 0) {
      ...
      tbodyRuang.innerHTML = ruangHariIni
        .map(
          (item, index) => `
                <tr>
                    <td>${index + 1}</td>
  ...
  if (tbodyBalai) {
    if (balaiHariIni.length === 0) {
      ...
      tbodyBalai.innerHTML = balaiHariIni
        .map(
          (item, index) => `
                <tr>
                    <td>${index + 1}</td>
  ...
  if (tbodyProgram) {
    if (programHariIni.length === 0) {
      ...
      tbodyProgram.innerHTML = programHariIni
        .map(
          (item, index) => `
                <tr>
                    <td>${index + 1}</td>
...
// CRUD JADWAL
// ============================================
function hapusJadwal(jenis, id, alreadyConfirmed = false) {
  const remove = () => {
    switch (jenis) {
      case "kunjungan":
  ...
  if (alreadyConfirmed) remove();
  else showAppConfirm("Yakin ingin menghapus jadwal ini?", remove);
}
...
function hapusJadwalFromModal() {
  ...
  if (!id) return;
  showAppConfirm("Yakin ingin menghapus jadwal ini?", () => {
    hapusJadwal(jenis, parseInt(id), true);
    closeJadwalModal();
  });
}
...
function switchMasterTab(tab) {
  ...
  const targetBtn = document.querySelector(
    `#masterModal .tab-btn[onclick="switchMasterTab('${tab}')"]`,
  );
  if (targetBtn) {
...
function renderMasterData() {
  ...
  if (listRuangan) {
    if (masterRuangan.length === 0) {
      listRuangan.innerHTML =
        '<p style="color:#888;font-size:13px;padding:8px 0;">Belum ada data ruangan</p>';
    } else {
      listRuangan.innerHTML = masterRuangan
        .map(
          (item) => `
  ...
  if (listTempat) {
    if (masterTempat.length === 0) {
      listTempat.innerHTML =
        '<p style="color:#888;font-size:13px;padding:8px 0;">Belum ada data tempat</p>';
    } else {
      listTempat.innerHTML = masterTempat
        .map(
          (item) => `
                <span class="master-item">
                    ${item}
  ...
  if (listPic) {
    if (masterPic.length === 0) {
      listPic.innerHTML =
        '<p style="color:#888;font-size:13px;padding:8px 0;">Belum ada data PIC</p>';
    } else {
      listPic.innerHTML = masterPic
        .map(
          (item) => `
                <span class="master-item">
                    ${item}
  ...
  if (listInstansi) {
    if (masterInstansi.length === 0) {
      listInstansi.innerHTML =
        '<p style="color:#888;font-size:13px;padding:8px 0;">Belum ada data instansi</p>';
    } else {
      listInstansi.innerHTML = masterInstansi
        .map(
          (item) => `
                <span class="master-item">
                    ${item}
...
function hapusMaster(jenis, value) {
  showAppConfirm(`Hapus "${value}" dari master ${jenis}?`, () => {
    switch (jenis) {
      case "ruangan":
...
function getEventStartKey(event) {
  if (event.tanggalMulai) return event.tanggalMulai;
  if (event.tanggal && event.bulan && event.tahun) {
    return formatDateKey(event.tahun, event.bulan - 1, event.tanggal);
  }
  return "";
}

function getEventEndKey(event) {
  return event.tanggalSelesai || getEventStartKey(event);
}

function eventOccursOnDate(event, dateKey) {
  const start = getEventStartKey(event);
  const end = getEventEndKey(event);
  return start && end && dateKey >= start && dateKey <= end;
}

function getEventsForCalendarDay(year, month, day) {
  const dateKey = formatDateKey(year, month, day);
  return eventData.filter((event) => eventOccursOnDate(event, dateKey));
}

function renderEventStrips(events, dateKey) {
  return events
    .map((event) => {
      const start = getEventStartKey(event);
      const end = getEventEndKey(event);
...
document.getElementById("monthYear").addEventListener("change", (event) => {
...
function showEventDetail(tanggal) {
  ...
  const selectedDate = tanggal ? formatDateKey(year, month - 1, tanggal) : "";
  const monthStart = formatDateKey(year, month - 1, 1);
  ...
  const events = eventData.filter((event) => {
    if (tanggal) return eventOccursOnDate(event, selectedDate);

    const eventStart = getEventStartKey(event);
    const eventEnd = getEventEndKey(event);
  ...
  if (events.length === 0) {
    if (tanggal) {
      ...
      container.innerHTML = `<p style="color:#888;">Belum ada event pada bulan ini.</p>`;
    }
    return;
  }
...
function lihatDetailEvent(id) {
  const event = eventData.find((item) => item.id === id);
  if (!event) return;

  const startKey = getEventStartKey(event);
  const [year, month, day] = startKey.split("-").map(Number);
  currentMonthFull = month - 1;
  currentYearFull = year;
  switchMenu("kalender");

  setTimeout(() => {
    renderCalendarFull(currentMonthFull, currentYearFull);
    showEventDetail(day);
    tampilkanDetailEvent(id);
  }, 100);
}
...
function tampilkanDetailEvent(id) {
  ...
  document.getElementById("detailEventWaktu").textContent = event.waktu || "-";
  document.getElementById("detailEventRuangan").textContent =
    event.ruangan || "-";
...
function tambahEvent() {
  ...
  document.getElementById("eventTanggalMulai").value = "";
  document.getElementById("eventTanggalSelesai").value = "";
...
function tambahEventTanggal(tanggal) {
  tambahEvent();
  const dateKey = formatDateKey(currentYearFull, currentMonthFull, tanggal);
  document.getElementById("eventTanggalMulai").value = dateKey;
  document.getElementById("eventTanggalSelesai").value = dateKey;
}
...
function editEvent(id) {
  ...
  document.getElementById("eventTanggalMulai").value = getEventStartKey(event);
  document.getElementById("eventTanggalSelesai").value = getEventEndKey(event);
...
function saveEvent() {
  ...
  if (id) {
    ...
    if (index !== -1) eventData[index] = event;
  } else {
...
function editEventSource(id, sumber) {
  const jenisMap = {
    Kunjungan: "kunjungan",
...
function hapusEvent() {
  const id = document.getElementById("eventId").value;
  if (!id) return;
  showAppConfirm("Yakin ingin menghapus event ini?", () => {
...
document.getElementById("monthYearFull").addEventListener("change", (event) => {
  if (!event.target.value) return;
  const [year, month] = event.target.value.split("-").map(Number);
  currentYearFull = year;
  currentMonthFull = month - 1;
...
function tambahCapaian() {
  document.getElementById("modalCapaianTitle").textContent =
    "Tambah Data Capaian";
...
function hapusCapaian() {
  ...
  if (index === "") return;
  showAppConfirm("Yakin ingin menghapus data capaian ini?", () => {
...
document.querySelectorAll("#page-dashboard .filter-btn").forEach((btn) => {
  btn.addEventListener("click", function () {
    ...
    this.classList.add("active");
    const period =
      this.dataset.period === "weekly"
        ? "mingguan"
        : this.dataset.period === "monthly"
          ? "bulanan"
          : "tahunan";
...
window.generateLaporanJS = function () {
  ...
  document.getElementById("lapTotalKunjungan").textContent = kunjunganData
    ? kunjunganData.length
    : 0;
  document.getElementById("lapTotalRuang").textContent = ruangData
    ? ruangData.length
    : 0;
  document.getElementById("lapTotalBalai").textContent = balaiData
    ? balaiData.length
    : 0;
  document.getElementById("lapTotalProgram").textContent = programData
    ? programData.length
    : 0;

  const now = new Date();
  document.getElementById("lapTanggalCetak").textContent =
  ...
  const jenisText =
    document.getElementById("laporanJenis").options[
      document.getElementById("laporanJenis").selectedIndex
    ].text;
  document.getElementById("lapJenisLaporan").textContent = jenisText;
  document.getElementById("laporanSummaryJenis").textContent =
    jenisText.replace(/^[^A-Za-z]+/u, "");
  document.getElementById("laporanSummaryKunjungan").textContent = kunjunganData
    ? kunjunganData.length
    : 0;
  document.getElementById("laporanSummaryRuang").textContent = ruangData
    ? ruangData.length
    : 0;
  document.getElementById("laporanSummaryProgram").textContent = programData
    ? programData.length
    : 0;
  document.getElementById("laporanPrintJenis").textContent = jenisText
    .replace(/^[^A-Za-z]+/u, "")
    .toUpperCase();
  document.getElementById("laporanPrintTanggal").textContent =
    `Dicetak pada ${document.getElementById("lapTanggalCetak").textContent}`;
  ...
  if (jenis === "all" || jenis === "kunjungan")
    totalData += kunjunganData.length;
  ...
  document.getElementById("laporanSummaryTotal").textContent =
    `${totalData} Data`;
...
function renderLaporanTable(jenis) {
  ...
  if (jenis === "all" || jenis === "kunjungan") {
    ...
    Object.keys(kunjunganBulanan)
      .sort()
      .forEach((key, index) => {
        const data = kunjunganBulanan[key];
        const bulanDisplay = data.bulan
          ? bulanFull[bulanNames.indexOf(data.bulan)] + " " + data.tahun
          : "Tidak Diketahui";
  ...
  rows.forEach((row) => {
    ...
    Object.values(row).forEach((val) => {
      if (
        typeof val === "string" &&
        (val.includes("✅") || val.includes("⚠️") || val.includes("❌"))
      ) {
...
function exportWordFile() {
  ...
  const content = `<!doctype html><html><head><meta charset="utf-8"><title>Laporan Penjadwalan</title></head><body><h1>Laporan Penjadwalan</h1>${table ? table.outerHTML : "<p>Tidak ada data laporan.</p>"}</body></html>`;
  const blob = new Blob([content], { type: "application/msword" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `Laporan_${new Date().toISOString().split("T")[0]}.doc`;
...
function resetAllData() {
  showAppConfirm(
    "⚠️ Yakin ingin menghapus SEMUA data? Ini tidak bisa dibatalkan!",
    () => {
      localStorage.clear();
      location.reload();
    },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (
    ...
    masterRuangan.includes("Lab. Komputer")
  ) {
